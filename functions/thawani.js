...
exports.thawaniWebhook = exports.listThawaniPaymentIntents = exports.verifyThawaniPayment = exports.createThawaniSession = void 0;
...
exports.createThawaniSession = functions.https.onCall(async (data, context) => {
    try {
        ...
        const { amount, currency = 'OMR', description = 'E-commerce purchase', customer, items = [], orderId, successUrl, cancelUrl, shippingAmount = 0, // ✅ Accept shipping amount separately
        ...
        if (items && items.length > 0) {
            calculatedAmount = items.reduce((sum, item) => {
            ...
            if (!amount) {
                throw new functions.https.HttpsError('invalid-argument', 'Either items or amount is required');
            }
        ...
        if (typeof finalAmount !== 'number') {
            throw new functions.https.HttpsError('invalid-argument', `Amount must be a number, received: ${typeof finalAmount}`);
        }
        if (isNaN(finalAmount)) {
            throw new functions.https.HttpsError('invalid-argument', 'Amount is NaN (Not a Number) - cannot process');
        }
        if (!Number.isFinite(finalAmount)) {
            throw new functions.https.HttpsError('invalid-argument', `Amount must be a finite number, received: ${finalAmount}`);
        }
        if (!Number.isInteger(finalAmount)) {
            throw new functions.https.HttpsError('invalid-argument', `Amount must be an integer (in baisa), received: ${finalAmount}. Convert using Math.round(omr * 1000)`);
        }
        if (finalAmount < 100) {
            throw new functions.https.HttpsError('invalid-argument', `Amount must be at least 100 baisa (0.1 OMR), received: ${finalAmount}`);
        }
        if (finalAmount > 100000000) { // 100,000 OMR
            throw new functions.https.HttpsError('invalid-argument', `Amount exceeds maximum limit (100,000 OMR), received: ${finalAmount} baisa`);
        }
        ...
        const products = items.length > 0
            ? items.map((item, index) => {
                ...
                if (!item || typeof item !== 'object') {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1}: must be an object, received: ${typeof item}`);
                }
                // 1. Validate name
                if (!item.name) {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1}: name is required`);
                }
                const nameStr = String(item.name).trim();
                if (nameStr === '') {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1}: name cannot be empty string`);
                }
                if (nameStr.length > 255) {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1}: name is too long (max 255 chars), received: ${nameStr.length}`);
                }
                // 2. Validate quantity - STRICT
                let quantity = item.quantity;
                if (quantity === undefined || quantity === null) {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1} "${nameStr}": quantity is required`);
                }
                // Convert to number first
                const quantityNum = Number(quantity);
                if (isNaN(quantityNum)) {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1} "${nameStr}": quantity must be a number, received: ${quantity}`);
                }
                // Convert to integer
                quantity = Math.floor(quantityNum);
                if (quantity < 1) {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1} "${nameStr}": quantity must be >= 1, received: ${item.quantity}`);
                }
                if (quantity > 10000) {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1} "${nameStr}": quantity exceeds limit (max 10000), received: ${quantity}`);
                }
                // 3. Validate and convert price - STRICT
                let price = item.price;
                if (price === undefined || price === null) {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1} "${nameStr}": price is required`);
                }
                // Convert to number
                const priceNum = Number(price);
                if (isNaN(priceNum)) {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1} "${nameStr}": price must be a number, received: ${price}`);
                }
                if (!Number.isFinite(priceNum)) {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1} "${nameStr}": price must be finite (not Infinity), received: ${price}`);
                }
                if (priceNum <= 0) {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1} "${nameStr}": price must be > 0, received: ${price}`);
                }
                // 4. Convert OMR to baisa (1 OMR = 1000 baisa) - STRICT
                const unitAmount = Math.round(priceNum * 1000);
                if (isNaN(unitAmount)) {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1} "${nameStr}": failed to convert price to baisa - NaN result`);
                }
                if (!Number.isInteger(unitAmount)) {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1} "${nameStr}": unit_amount must be integer after conversion, got: ${unitAmount}`);
                }
                if (unitAmount < 100) {
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1} "${nameStr}": unit_amount must be >= 100 baisa (0.1 OMR), calculated: ${unitAmount} from price ${price}`);
                }
                if (unitAmount > 10000000) { // 10,000 OMR per item
                    throw new functions.https.HttpsError('invalid-argument', `Product ${index + 1} "${nameStr}": unit_amount exceeds limit (10,000 OMR), calculated: ${unitAmount} baisa`);
                }
        ...
        if (amountDifference > TOLERANCE_BAISA) {
            ...
            throw new functions.https.HttpsError('invalid-argument', `Order calculation error: product total (${calculatedTotal} baisa = ${(calculatedTotal / 1000).toFixed(3)} OMR) does not match requested amount (${finalAmount} baisa = ${(finalAmount / 1000).toFixed(3)} OMR). ` +
        ...
        if (!Number.isInteger(finalAmount)) {
            ...
            throw new functions.https.HttpsError('invalid-argument', `Total amount must be an integer in baisa, received: ${finalAmount}`);
        }
        if (isNaN(finalAmount)) {
        ...
        if (!Number.isFinite(finalAmount)) {
            ...
            throw new functions.https.HttpsError('invalid-argument', `Total amount must be finite, received: ${finalAmount}`);
        }
        ...
        if (finalAmount < productsTotal) {
            ...
            throw new functions.https.HttpsError('invalid-argument', 'Order total is less than the sum of product prices');
        }
        ...
        if (sessionId === undefined || sessionId === null) {
            ...
            throw new functions.https.HttpsError('internal', 'Thawani API did not return a session ID');
        }
        ...
        try {
            const paymentSessionDoc = {
                ...
                // ⏱️ Dates
                createdAt: admin.firestore.Timestamp.fromDate(new Date(thawaniData.created_at || new Date())),
                expiresAt: admin.firestore.Timestamp.fromDate(new Date(thawaniData.expire_at || new Date(Date.now() + 30 * 60 * 1000))),
        ...
        if (error.response?.status === 401) {
            throw new functions.https.HttpsError('permission-denied', 'Payment gateway rejected API key - check configuration');
        }
        if (error.response?.status === 400) {
            const apiMessage = error.response.data?.message || error.response.data?.error || '';
            const fullErrorData = JSON.stringify(error.response.data);
            throw new functions.https.HttpsError('invalid-argument', `Invalid payment request from Thawani: ${apiMessage || fullErrorData || 'Bad request'}`);
        }
        if (error.response?.status === 403) {
            throw new functions.https.HttpsError('permission-denied', 'API key does not have permission - check Thawani configuration');
        }
        if (error.response?.status === 503) {
            console.error('❌ [THAWANI] 503 Service Unavailable');
            throw new functions.https.HttpsError('unavailable', 'Payment gateway is temporarily unavailable. Please try again later.');
        }
        ...
        if (error.code === 'ECONNREFUSED') {
            ...
            throw new functions.https.HttpsError('unavailable', 'Cannot reach payment gateway. Please check your connection and try again.');
        }
        if (error.code === 'ENOTFOUND') {
            console.error('❌ [THAWANI] DNS error - gateway not reachable');
            throw new functions.https.HttpsError('unavailable', 'Payment gateway is unreachable. Please try again later.');
        }
 ...
 * }
 */
exports.listThawaniPaymentIntents = functions.https.onCall(async (data, context) => {
    try {
        // Verify user is authenticated
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
        }
        const { limit = 10, skip = 0, clientReferenceId, } = data;
        // Validate limit
        ...
        if (typeof skip !== 'number' || skip < 0) {
            throw new functions.https.HttpsError('invalid-argument', 'Skip must be a non-negative number');
        }
        const thawaniApiKey = runtimeConfig.thawani?.secret;
        if (!thawaniApiKey) {
            console.error('Thawani API key not configured');
        ...
        // Call Thawani API
        const response = await axios_1.default.get(`${THAWANI_BASE_URL}/payment_intents`, {
            params: {
                limit,
                skip,
            },
        ...
        if (!response.data || !Array.isArray(response.data.data)) {
            ...
            throw new functions.https.HttpsError('internal', 'Failed to retrieve payment intents from gateway');
        }
        // Filter by clientReferenceId if provided
        let intents = response.data.data;
        if (clientReferenceId) {
            intents = intents.filter((intent) => intent.client_reference_id === clientReferenceId);
        ...
        if (error.response?.status === 400) {
            ...
            throw new functions.https.HttpsError('invalid-argument', `Invalid request to payment gateway: ${apiMessage}`);
        }
        if (error.code === 'ECONNABORTED') {
            throw new functions.https.HttpsError('deadline-exceeded', 'Request to payment gateway timed out');
        }
        throw new functions.https.HttpsError('internal', 'Failed to list payment intents');
    }
});
/**
