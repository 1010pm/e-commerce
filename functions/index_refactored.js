...
exports.health = exports.deletePaymentMethod = exports.updatePaymentMethod = exports.createPaymentMethod = exports.stripeWebhook = exports.processPayment = void 0;
...
exports.processPayment = functions.https.onCall(async (data, context) => {
    try {
        ...
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated to process payments');
        }
        ...
        switch (provider.toLowerCase()) {
            case 'stripe':
                result = await handleStripePayment(amount, currency, paymentData, { userId, methodId, ...metadata });
                break;
            case 'paypal':
                result = await handlePayPalPayment(amount, currency, paymentData, { userId, methodId, ...metadata });
                break;
            case 'bank_transfer':
            ...
            default:
                ...
                if (paymentMethod?.provider === provider) {
                    result = await handleCustomPayment(provider, amount, currency, paymentData, { userId, methodId, ...metadata });
                }
                else {
                    throw new functions.https.HttpsError('invalid-argument', `Payment provider '${provider}' is not configured`);
                }
        }
        // Log transaction
...
// ============================================
async function handleStripePayment(amount, currency, paymentData, metadata) {
    try {
...
// ============================================
async function handlePayPalPayment(amount, currency, paymentData, metadata) {
    try {
...
// ============================================
async function handleBankTransferPayment(amount, currency, metadata) {
    try {
...
// ============================================
async function handleCustomPayment(provider, amount, currency, paymentData, metadata) {
    try {
...
exports.createPaymentMethod = functions.https.onCall(async (data, context) => {
    try {
        ...
        const { name, provider, description, publicConfig = {}, fields = [], isActive = true, logo = null, } = data;
        // Validate required fields
        if (!name || !provider) {
        ...
        if (hasSecretValues(publicConfig) || hasSecretValues(data)) {
            throw new functions.https.HttpsError('invalid-argument', 'SECURITY ERROR: Secret keys cannot be submitted via UI. Configure secrets via Cloud Functions environment variables.');
        }
        ...
        if (existingMethods.size >= 20) {
            throw new functions.https.HttpsError('resource-exhausted', 'Maximum payment methods limit reached');
        }
        ...
        throw new functions.https.HttpsError('internal', error.message || 'Failed to create payment method');
    }
});
/**
...
exports.updatePaymentMethod = functions.https.onCall(async (data, context) => {
    try {
        ...
        if (!methodId) {
            throw new functions.https.HttpsError('invalid-argument', 'Method ID is required');
        }
        ...
        if (hasSecretValues(updates)) {
            throw new functions.https.HttpsError('invalid-argument', 'SECURITY ERROR: Secret keys cannot be updated via UI. Configure secrets via Cloud Functions environment variables.');
        }
        // Update method
        await db
            .collection('paymentMethods')
        ...
        throw new functions.https.HttpsError('internal', error.message || 'Failed to update payment method');
    }
});
/**
...
//# sourceMappingURL=index_refactored.js.map